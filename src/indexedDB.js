...
const db = {
    ...
    keys(store) {
        return dbPromise.then(db => {
            ...
            (storeFallback.iterateKeyCursor || storeFallback.iterateCursor).call(storeFallback, cursor => {
                if (!cursor) return;
...
db.set('foo', 'genius', 'users'); 
db.set('babe', 'barbie', 'items');
db.set('food', 'rice', 'orders');
